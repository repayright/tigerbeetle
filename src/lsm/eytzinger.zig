        ...
        pub fn search_values(
            ...
            while (i < keys.len) {
                if (comptime prefetch) {
                    ...
                    assert(constants.cache_line_size > @sizeOf(Key));

                    // Prefetching next levels from memory.
                    // https://arxiv.org/abs/1509.05053
                    const cache_line_keys =
                        comptime @divExact(constants.cache_line_size, @sizeOf(Key));
                    ...
                    @prefetch(keys.ptr + (cache_line_keys * i) + cache_line_keys, .{
                        .rw = .read,
                        .locality = 0,
                        .cache = .data,
                    });
